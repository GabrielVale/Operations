...
##NO CASO DAS LOJAS ONE NÃO ESQUECER DE ALTERAR AS VARIÁVEIS $Loja, $IP, $One, $Time, $ftpUser, $ftpUserImg, $ftpisolate
...
#Criando WebSite
New-Website -Name $Loja -Port 80 -IPAddress $IP -PhysicalPath E:\IIS\wwwroot\$One -ApplicationPool $Loja 
...
Add-WebConfiguration -Filter /system.webserver/security/ipsecurity -Value @{ipAddress="177.154.153.0"; subnetMask="255.255.255.0"; allowed="true"} -Location $Loja -PSPath "IIS:\Sites"
Add-WebConfiguration -Filter /system.webserver/security/ipsecurity -Value @{ipAddress="177.154.155.0"; subnetMask="255.255.255.0"; allowed="true"} -Location $Loja -PSPath "IIS:\Sites"
Add-WebConfiguration -Filter /system.webserver/security/ipsecurity -Value @{ipAddress="177.154.156.0"; subnetMask="255.255.252.0"; allowed="true"} -Location $Loja -PSPath "IIS:\Sites"
...
New-Item IIS:\Sites\$Loja\IKCAdm -physicalPath E:\IIS\wwwroot\$One\IKCAdm -type Application
Set-ItemProperty IIS:\Sites\$Loja\IKCAdm -name applicationPool -value $Loja'_IKCAdm'

#New-Item IIS:\Sites\$Loja\IKCTelevendas -physicalPath E:\IIS\wwwroot\$One\IKCTelevendas -type Application
#Set-ItemProperty IIS:\Sites\$Loja\IKCTelevendas -name applicationPool -value $Loja'_IKCTelevendas'

New-Item IIS:\Sites\$Loja\IKCWebService -physicalPath E:\IIS\wwwroot\$One\IKCWebService -type Application
